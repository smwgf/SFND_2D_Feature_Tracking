...
{
    for(vector<cv::KeyPoint>::iterator iter = keypoints.begin(); iter !=keypoints.end();)
    {
        auto x = (*iter).pt.x;
        auto y = (*iter).pt.y;
        if(x < filtRect.x || x > filtRect.x+filtRect.width || y <filtRect.y || y > filtRect.y+filtRect.height)
        {
